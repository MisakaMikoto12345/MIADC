...
def get_input_template(user_prompt,
    ...
    if 'llama' in model_name:
        for i in range(target_stop):
            if system_prompt in tokenizer.decode(input_ids[:i]) :
                user_prompt_start = i + 8
                break

    elif 'zephyr' in model_name:
        for i in range(target_stop):
            if system_prompt in tokenizer.decode(input_ids[:i]) :
                user_prompt_start = i + 9
                break

    elif 'vicuna' in model_name:
        for i in range(target_stop):
            if system_prompt in tokenizer.decode(input_ids[:i]) :
                user_prompt_start = i + 4
                break
    ...
    for i in range(target_stop, 0, -1):
        if tokenizer.decode(input_ids[i:]) == target_response:
            ...
            user_prompt_stop = adv_start
            break
    # for i in range(target_stop, 0, -1):
    #     decoded = tokenizer.decode(input_ids[i:])
    #     if decoded == target_response:
    #         target_start = i
    #     elif decoded.endswith(target_response):
    #         target_start = i + len(tokenizer.encode(
    #             decoded[:-len(target_response)],
    #             add_special_tokens=False
    #         ))
    #     elif adv_tokens[1:] in decoded:
    #         adv_start = i
    #         adv_stop = i + len_adv_tokens
    #         user_prompt_stop = adv_start
    #         break
    ...
    slices = {
        ...
        "user_prompt_slice": slice(user_prompt_start, user_prompt_stop)
    }
